...
import { PromptGenerationRequest, ComplexityLevel } from '../types/promptTypes';

export class MetadataGenerator {
  assessComplexity(featureCount: number): ComplexityLevel {
  ...
  estimateBuildTime(featureCount: number, complexity: ComplexityLevel): string {
    const baseTime = {
      'Simple': 2,
      'Moderate': 5,
      'Complex': 10,
    ...
    const days = baseTime[complexity] || 5;
    return `${days}-${days + 3} days`;
  }

  generateComponentSuggestions(request: PromptGenerationRequest): string[] {
  ...
  generateMCPEndpoints(request: PromptGenerationRequest): string[] {
  ...
  generateA2AProtocols(request: PromptGenerationRequest): string[] {
  ...
  generateRAGPipeline(request: PromptGenerationRequest): string {
    const components = [
